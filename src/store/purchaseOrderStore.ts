...
interface PurchaseOrdersState {
	...
	createPurchaseOrder: (newPurchaseOrder: PurchaseOrder) => Promise<ApiResponse>;
	editPurchaseOrderById: (newPurchaseOrder: PurchaseOrder) => Promise<ApiResponse>;
...
export const usePurchaseOrdersStore = create<PurchaseOrdersState>((set, get) => ({
	...
	fetchPurchaseOrders: async (page = 1) => {
		...
		try {
			...
			set({ purchaseOrders, totalPages, currentPage, loading: false });
		} catch (error) {
	...
	getPurchaseOrderById: (id: string): PurchaseOrder | undefined => {
		...
		const purchaseOrder = purchaseOrders.find((purchaseOrder) => purchaseOrder._id === id);
		if (purchaseOrder) return purchaseOrder;
		else return undefined;
	}, //getPurchaseOrderById
	createPurchaseOrder: async (newPurchaseOrder: PurchaseOrder): Promise<ApiResponse> => {
		set({ loading: true });
		try {
			const purchaseOrder = await createPurchaseOrder(newPurchaseOrder);
			if (purchaseOrder) {
				set((state) => ({
	...
	editPurchaseOrderById: async (purchaseOrder: PurchaseOrder): Promise<ApiResponse> => {
		set({ loading: true });
		try {
			...
			if (response) {
				set((state) => ({
					purchaseOrders: state.purchaseOrders.map((item) =>
						item._id === purchaseOrder._id ? purchaseOrder : item
					),
				}));
				set({ loading: false });
				return {
